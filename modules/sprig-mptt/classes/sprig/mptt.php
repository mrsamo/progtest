...
abstract class Sprig_MPTT extends Sprig
{
    ...
    {
        Database::instance($this->_db)->query(NULL, 'LOCK TABLE ' . Database::instance($this->_db)->table_prefix() . $this->_table . ' WRITE', TRUE);
    }
...
//				$array[$row[$key]] = str_repeat($indent, $row[$this->level_column]).$row[$value];
                ...
                if ($this->id == $row[$key])
                    $selected = TRUE;
                $array[] = array('id' => $row[$key], 'name' => str_repeat($indent, $row[$this->level_column]) . $row[$value], 'selected' => $selected);
            }
...
//        $this->lock();
        ...
        try
        {
            ...
            // Update the values.
            Database::instance($this->_db)->query(NULL, 'UPDATE ' . Database::instance($this->_db)->table_prefix() . $this->_table . ' 
    ...
    public function reload()
    {
        ...
        $mptt_vals = DB::select(
                        $this->left_column, $this->right_column, $this->level_column, $this->scope_column
                )
                ->from($this->_table)
                ->where($this->pk(), '=', $this->{$this->pk()})
                ->execute($this->_db)
    ...
    public function select_list_form()
    {
        return $this->select_list($key = 'id', $value = 'name', '-');
    }

    /**
     */
    public function is_last_in_level($self = FALSE, $direction = 'DESC')
    {
        $nodes = $this->siblings($self = FALSE, $direction = 'ASC');
        return count($nodes);
    }
